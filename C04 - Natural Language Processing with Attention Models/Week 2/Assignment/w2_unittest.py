...
def test_decoderlayer(target, create_look_ahead_mask):
    def g():
        ...
        t = test_case()
        if not np.allclose(attn_w_b2[0, 0, 1], [0.25345066 , 0.29631895 , 0.4502304 ]):
            t.failed = True
            t.msg = "Wrong values in 'attn_w_b2'. Check the call to self.mha2"
            t.want = [0.25345066 , 0.29631895 , 0.4502304 ]
            t.got = attn_w_b2[0, 0, 1]
        cases.append(t)      
    
        t = test_case()
        if not np.allclose(out[0, 0], [ 1.4810526 , -1.0550661 , -0.7618928  , 0.33590618]):
            t.failed = True
            t.msg = "Wrong values in 'out'"
            t.want = [ 1.4810526 , -1.0550661 , -0.7618928  , 0.33590618]
            t.got = out[0, 0]
        cases.append(t)  
        ...
        t = test_case()
        if not np.allclose(out[0, 0], [ 1.4630504 , -0.9013529 , -0.9476653  , 0.38596773]):
            t.failed = True
            t.msg = "Wrong values in 'out' when we mask the last word. Are you passing the padding_mask to the inner functions?"
            t.want = [ 1.4630504 , -0.9013529 , -0.9476653  , 0.38596773]
            t.got = out[0, 0]
        cases.append(t)  
...
def test_decoder(target, create_look_ahead_mask, create_padding_mask):
    def g():
        ...
        outd, att_weights = decoderk(x = x, enc_output = encoderq_output, training = False, look_ahead_mask = look_ahead_mask, padding_mask = None)
        ...
        t = test_case()
        if not np.allclose(outd[1, 1], [ 1.1661551 , 0.5968947 , -1.485516 ,  -0.27753386]):
            t.failed = True
            t.msg = "Wrong values in x"
            t.want = [ 1.1661551 , 0.5968947 , -1.485516 ,  -0.27753386]
            t.got = outd[1, 1]
        cases.append(t)   
        ...
        t = test_case()
        if not np.allclose(att_weights[keys[0]][0, 0, 1], [0.5049869, 0.4950131, 0.0]
):
            t.failed = True
            t.msg = f"Wrong values in att_weights[{keys[0]}]"
            t.want = [0.5049869, 0.4950131, 0.0]
            t.got = att_weights[keys[0]][0, 0, 1]
        cases.append(t)
        
        outd, att_weights = decoderk(x = x, enc_output = encoderq_output, training = True, look_ahead_mask = look_ahead_mask, padding_mask = None)
        
        t = test_case()
        if not np.allclose(outd[1, 1], [0.05685997, -1.0754473, -0.5680678, 1.586655]):
            t.failed = True
            t.msg = "Wrong values in outd when training=True"
            t.want = [0.05685997, -1.0754473, -0.5680678, 1.586655]
            t.got = outd[1, 1]
        cases.append(t)
        
        outd, att_weights = decoderk(x = x, enc_output = encoderq_output, training = True, look_ahead_mask = look_ahead_mask, padding_mask = create_padding_mask(x))
        
        t = test_case()
        if not np.allclose(outd[1, 1], [0.18430379, -0.8958332, -0.8505456, 1.5620749]):
            t.failed = True
            t.msg = "Wrong values in outd when training=True and use padding mask"
            t.want = [0.18430379, -0.8958332, -0.8505456, 1.5620749]
            t.got = outd[1, 1]
        cases.append(t)
...
def test_transformer(target, create_look_ahead_mask, create_padding_mask):
    ...
    dec_padding_mask = dec_padding_mask
        ...
        summary_example_1 = [0.0284844, 0.03627992, 0.02426082, 0.02042055, 0.03156459, 0.0317674, 0.01480251, 0.03056922]
        ...
        t = test_case()
        if not np.allclose(weights[keys[0]][0, 0, 1], [0.48929223, 0.5107078, 0.0, 0.0, 0.0]
):
            t.failed = True
            t.msg = f"Wrong values in weights[{keys[0]}]"
            t.want = [0.48929223, 0.5107078, 0.0, 0.0, 0.0]
    ...
    dec_padding_mask = dec_padding_mask
        ...
        summary_example_2 = [0.02850271, 0.03705186, 0.02455955, 0.02084055, 0.03113986, 0.03163682, 0.01485186, 0.0314099]
