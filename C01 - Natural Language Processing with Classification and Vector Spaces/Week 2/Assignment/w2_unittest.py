...
def test_train_naive_bayes(target, freqs, train_x, train_y):
    ...
    for test_case in test_cases:
        ...
        for key, value in test_case["expected"]["loglikelihood"].items():
            ...
            if np.isclose(result2.get(key,0), value):
                count_good += 1
...
def test_naive_bayes_predict(target):
  ...
  'input': {'tweet': 'She smiled.',
   'logprior': 0.0,
   'loglikelihood': pickle.load(open("./support_files/loglikelihood_test.pkl", "rb"))},
  'expected': 1.554573505228165},
 {'name': 'neutral_example_check',
  'input': {'tweet': 'She did not answer my question.',
   'logprior': 0.0,
   'loglikelihood': pickle.load(open("./support_files/loglikelihood_test.pkl", "rb"))},
  'expected': 0.5960694837016335},
 {'name': 'negative_example_check',
  'input': {'tweet': 'She did not answer my question :(',
   'logprior': 0.0,
   'loglikelihood': pickle.load(open("./support_files/loglikelihood_test.pkl", "rb"))},
  'expected': -6.934406231899679},
 {'name': 'positive_prior_check',
  'input': {'tweet': 'She smiled.',
   'logprior': 0.6931471805599456,
   'loglikelihood': pickle.load(open("./support_files/loglikelihood_test.pkl", "rb"))},
  'expected': 2.247720685788111},
 {'name': 'neutral_example_prior_check',
  'input': {'tweet': 'She did not answer my question.',
   'logprior': 0.6931471805599456,
   'loglikelihood': pickle.load(open("./support_files/loglikelihood_test.pkl", "rb"))},
  'expected': 1.2892166642615792},
 {'name': 'negative_example_prior_check',
  'input': {'tweet': 'She did not answer my question :(',
   'logprior': 0.6931471805599456,
   'loglikelihood': pickle.load(open("./support_files/loglikelihood_test.pkl", "rb"))},
  'expected': -6.241259051339734}]

    for test_case in test_cases:
        result = target(**test_case["input"])

        try:
            assert np.isclose(result, test_case["expected"],atol=1e-02)
            successful_cases += 1
        except:
            failed_cases.append(
                {
...
def test_get_words_by_threshold(target, freqs):
    ...
    test_cases = [{'name': 'default_check1',
  'input': {'freqs': freqs, 'label': 0, 'threshold': 0.05},
  'expected': {':(': {'positive': 1, 'negative': 3675, 'ratio': 0.000544069640914037},
 ...
 {'name': 'high_threshold_check',
  ...
  'expected': {':)': {'positive': 2960, 'negative': 2, 'ratio': 987.0},
